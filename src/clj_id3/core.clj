...
(defn normalize-str [string]
  (-> string
      (Normalizer/normalize java.text.Normalizer$Form/NFKC)))

(defn normalize-id3 [data]
  (let [{:keys [mp3 file
                track artist title album original-artist album-artist
                year genre genre-description original-artist album-artist
                part-of-set album-image album-image-mime-type]} data]
    (if-not (and track title album (or artist album-artist))
      ...
      (try
        (let [id3 (ID3v24Tag.)
          ...
          (when part-of-set (.setPartOfSet id3 part-of-set))
          ...
          data)
        (catch Exception e
          (println file "throw erros in creating tag.")
          (.printStackTrace e))))))
    ...
    (let [mp3 (Mp3File. file)
      ...
      (if (.hasId3v2Tag mp3)
        (cond-> {}
          ...
          (.getPartOfSet id3) (assoc :part-of-set (.getPartOfSet id3))
...
(defn escape-name [str]
  (str/replace str #"/|／" "\\\\"))

(defn save-mp3 [data]
  (let [{:keys [mp3 id3 file
                track artist title album album-artist part-of-set]} data
        out (io/file "output"
                     (escape-name (or album-artist artist))
                     (escape-name album)
                     (str
                      (when part-of-set
                        (-> part-of-set
    ...
    (try
      (if (.exists out)
        (println (.getAbsolutePath out) "exists.")
